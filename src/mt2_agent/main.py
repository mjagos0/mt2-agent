...
def main():
    ...
    if args.no_gui:
        ...
        bot_thread = threading.Thread(target=agent.run_loop, daemon=True)
        bot_thread.start()
...
def handle_args() -> argparse.Namespace:
    ...
    features.add_argument("--pickup-interval", type=float, default=1, help="Item pickup interval in seconds")
    ...
    # Stuck detection
    stuck_settings.add_argument("--unstuck-threshold", type=int, default=60,
        help="Number of seconds to remain in the same place to execute unstuck procedure")
    stuck_settings.add_argument("--unstuck-clicks", type=int, default=30,
    ...
    # Auto target
    autotarget_settings.add_argument("--target-boss", type=int, default=0, help="Auto target boss priority. 0 for off.")
    autotarget_settings.add_argument("--target-boulder", type=int, default=3, help="Auto target boulder priority. 0 for off.")
    autotarget_settings.add_argument("--target-enemy", type=int, default=0, help="Auto target enemy priority. 0 for off.")
    autotarget_settings.add_argument("--target-random", type=int, default=0, help="Auto target random spot priority. 0 for off.")

    # Captcha
    captcha_settings.add_argument("--captcha-trigger-template-path", type=str,
        default="assets/nothyr/captcha-detect.png", help="Path to Captcha trigger image template")

    # Input settings
    input_settings.add_argument("--rebind", nargs=2, action="append", metavar=("NAME", "KEY"),
        help="Rebind a hotkey, e.g. --rebind HOTKEY_1 q --rebind PICKUP_ITEMS x")
    input_settings.add_argument("--input-delay", type=float, default=0.06, help="Average delay between agent inputs")

    # Detection model
    model_settings.add_argument("--obj-model-path", type=str, default="assets/model/best.pt", help="Path to YOLOv11 model weights")
    model_settings.add_argument("--obj-model-confidence-cutoff", type=float, default=0.3, help="Detection confidence threshold")

    # Assets
    assets_settings.add_argument("--asset-icon-dir", type=str, default="assets/icons", help="Path to folder with game icons")
    assets_settings.add_argument("--screenshot-path", default=DEFAULT_SCREENSHOT_PATH, help="Redefine relative screenshot path")

    # Developer
    developer_settings.add_argument("--debug", action="store_true", help="Show developer logs")
    developer_settings.add_argument("--debug-folder", default="debug", help="Developer folder for debugging")
    developer_settings.add_argument("--debug-folder-screenshots", default="debug", help="Developer folder for debug screenshots")

    # GUI
    p.add_argument("--no-gui", action="store_true", help="Run without the control panel GUI")
...
class MetinAgent:
    def __init__(self, args: argparse.Namespace, game: GameInterface):
        ...
        self.all_tasks: dict[str, ScheduledTask] = {}
    ...
    def _schedule(self, name: str, action_fn: Callable[..., None], interval: float,
        ...
        logger.info(f"{name} scheduled every {interval}s (first in {initial_delay}s)")
        task = ScheduledTask(
    ...
    def run_loop(self):
        """Bot loop intended to run in a background thread."""
        while self._should_run():
            ...
            if task.enabled:
                ...
                logger.debug("Skipping disabled task '%s'", task.name)

            task.next_run = time.monotonic() + task.interval
            heapq.heappush(self._heap, task)

    def run(self):
        """Original single-threaded entry point (--no-gui mode)."""
        self.setup_schedule()
        self.run_loop()
